...
var cert = fromPEM(fs.readFileSync('/home/ohtsu/tmp/cert/iijplus/iijplus.jp.cert'));
var ca = fromPEM(fs.readFileSync('/home/ohtsu/tmp/cert/iijplus/iijplus.jp.ca'));
var key = fs.readFileSync('/home/ohtsu/tmp/cert/iijplus//iijplus.jp.key');

var initial_version = new Buffer('0303', 'hex'); // TLS1.2
var initial_cipher = new Buffer('009C', 'hex');  // TLS_RSA_WITH_AES_128_GCM_SHA256

var port = 443;
var server = net.createServer(function(s) {
  var tlsConnection = new TLSConnection(s, true);
...
function TLSConnection(socket, is_server) {
  ...
  this.on('ChangeCipherSpec', function() {
    ...
    this.state.client_write_key = key_block.slice(0, 16);
    this.state.server_write_key = key_block.slice(16, 32);
    this.state.client_write_IV =  key_block.slice(32, 36);
    this.state.server_write_IV = key_block.slice(36, 40);
...
TLSFrame.prototype.processClientKeyExchange = function(reader) {
  ...
  var preMasterSecret = crypto.privateDecrypt({key:key, padding: constants.RSA_PKCS1_PADDING}, encryptedPreMasterSecret);
